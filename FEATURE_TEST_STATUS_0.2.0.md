...
## Requirements before expanding a feature to another build

A feature should move from unavailable or experimental to profiled only after:

1. The exact executable identity is recorded.
2. Every required structure field stays inside the measured object.
3. Every internal call has a verified address and x86 calling convention.
4. Every code patch checks the original bytes before writing.
5. Failure and disconnect behavior is defined, including rollback where
   practical.
6. The feature is tested in the relevant menu or board state on that exact
   executable.
7. Special cases such as bosses, beta IDs, minigames, pool/roof rows, and
   transitions are either tested or explicitly blocked.
8. The compatibility document and capability reason are updated without
   claiming feature equality with another build.

## Reporting useful 0.2.0 results

Include:

- Exact game version, region, distribution, and detected signature.
- Windows version and whether the game runs through WoW64.
- Trainer language and backend.
- Feature, values used, and whether the action occurred in a menu, seed
  chooser, or active board.
- Whether the effect was immediate or required reopening a menu or board.
- Exact reproduction steps, relevant trainer log, and screenshots.
- Whether PvZ, the trainer, or the save was restarted after the failure.

Do not attach copyrighted game binaries or personal save files.
