...
## 11. Reproduction and developer workflow

The shipped trainer does not require MASM or Cheat Engine. Those tools were used
during research and payload generation only. The generated Python module contains the
finalized machine-code blob, relocation list, symbol offsets and preferred pointer
cells.

| Artifact | Purpose |
| --- | --- |
| `docs/QUICKPLAY_RESEARCH.md` | Chronological evidence log, discarded hypotheses and dynamic observations. |
| `tools/quickplay_native_patch.asm` | Readable source of the 32-bit native payload. |
| `tools/build_quickplay_payload.py` | COFF parser and generator for the embedded Python payload module. |
| `pvzdbg/quickplay_payload.py` | Generated bytes, relocations, symbols and preferred pointers used by distributed builds. |
| `pvzdbg/quickplay.py` | Validation, linking, install, maintenance, re-entry, restoration and memory release. |
| `pvzdbg/backends/windows_native.py` | Backend-facing start, stop, return and maintain methods. |
| `pvzdbg/ui.py` | Warning, activation, Return to Quick Play, restoration controls and 100 ms maintenance scheduling. |
| `tests/test_quickplay.py` | Deterministic lifecycle and safety regression tests. |

### 11.1 Payload regeneration

1. Assemble `tools/quickplay_native_patch.asm` as 32-bit COFF.
2. Parse `.text$mn`, the COFF symbol table and DIR32 relocations.
3. Resolve internal absolute references into payload-relative values.
4. Record every relocation that must receive the remote base.
5. Extract named pointer-cell defaults and selected public symbol offsets.
6. Write `pvzdbg/quickplay_payload.py`.
7. Run focused Quick Play tests and the complete repository test suite.
8. Perform a live smoke test only on the exact validated executable.

Any change to payload length, hook prologues, resource assumptions or structure
offsets should be treated as a new validation event. A successful assembly is not
...
| --- | --- | --- |
| 1 | Crash-resilient restoration journal | Record original temporary profile values outside the target process and offer recovery after an interrupted session. |
| 2 | Additional GOTY build research | Independent signatures, structures, resources and live tests for each exact regional revision. |
| 3 | Adventure coverage matrix | Document expected results for levels 1-50 across representative profile progression states. |
| 4 | Read-only diagnostics panel | Expose validated root, page, pending restore, selected level and Board observation without showing unsafe manual writes. |
| 5 | Automated live smoke harness | Launch a disposable profile, exercise entry/category/back paths and verify hook cleanup on the supported build. |
| 6 | Visual fidelity study | Determine whether usable Survival and Puzzles highlight frames can be reconstructed from original atlas metadata without unsafe objects. |
